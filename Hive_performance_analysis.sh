...
END {
    print "Log start time : " start_time[1]
    print "Log end time : "   stop_time[1]
    print "Last query parsed at : "  last_query_time
    ...
    for (b_user in beeline_sessions_by_user)
      if(cntr<3){
        printf b_user " : "beeline_sessions_by_user[b_user]"  "
        cntr++
        }
    print " "
    print "Total number of Beeline sessions : " number_of_beeline_sessions
    ...
    for (m_user in metastore_hits_user)
      if(user_cntr<3){
        printf m_user " : "metastore_hits_user[m_user]"  "
        user_cntr++
        }
    print " "
    ...
    for (m_cmd in metastore_hits_command)
      if(cmd_cntr<3){
        printf m_cmd " : "metastore_hits_command[m_cmd]"  "
        cmd_cntr++
        }

    print " "
    print "Total Number of Metastore hits : " total_metastore_hits
    print "\nNumber of Total Queries : " number_of_queries_total
    print "\n====Total time spent in each method, Min, Max, Mean of time spent per query per method====\n"
		...
		print "     PreHook.org.apache.hadoop.hive.ql.hooks.ATSHook " duration_per_method["PreHook.org.apache.hadoop.hive.ql.hooks.ATSHook"] min_duration_per_method["PreHook.org.apache.hadoop.hive.ql.hooks.ATSHook"],max_duration_per_method["PreHook.org.apache.hadoop.hive.ql.hooks.ATSHook"],\
		...
		print "     PreHook.org.apache.hadoop.hive.ql.security.authorization.plugin.DisallowTransformHook " duration_per_method["PreHook.org.apache.hadoop.hive.ql.security.authorization.plugin.DisallowTransformHook"] min_duration_per_method["PreHook.org.apache.hadoop.hive.ql.security.authorization.plugin.DisallowTransformHook"],max_duration_per_method["PreHook.org.apache.hadoop.hive.ql.security.authorization.plugin.DisallowTransformHook"],\
		...
		print "    PostHook.org.apache.hadoop.hive.ql.hooks.ATSHook " duration_per_method["PostHook.org.apache.hadoop.hive.ql.hooks.ATSHook"] min_duration_per_method["PostHook.org.apache.hadoop.hive.ql.hooks.ATSHook"],max_duration_per_method["PostHook.org.apache.hadoop.hive.ql.hooks.ATSHook"],\
